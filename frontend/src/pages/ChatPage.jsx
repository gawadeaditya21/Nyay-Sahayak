...
import { useState, useRef, useEffect } from 'react';
import { 
  Upload, ArrowUp, FileText, Loader2, X, 
  Shield, Gavel, AlertCircle 
} from 'lucide-react';
import { analyzeDocument, analyzeText } from '../services/api';
import { formatAnalysisResponse } from '../utils/formatAnalysis';
...
export default function ChatPage() {
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // STATE MANAGEMENT (ALL ORIGINAL LOGIC RETAINED)
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  const [file, setFile] = useState(null);
  const [inputText, setInputText] = useState("");
  const [loading, setLoading] = useState(false);
  ...
  const handleFileChange = (e) => {
    if (e.target.files[0]) {
      ...
      if (!allowedTypes.includes(selectedFile.type)) {
        addErrorMessage("Invalid file type. Please upload a PDF or image file.");
        return;
      }
      setFile(selectedFile);
    }
  ...
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // CHAT MANAGEMENT LOGIC (UNTOUCHED)
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  const addUserMessage = (content, hasFile = false) => {
    setChatHistory(prev => [...prev, { role: 'user', content, hasFile, timestamp: new Date().toISOString() }]);
  };

  const addAIMessage = (content, isError = false) => {
    setChatHistory(prev => [...prev, { role: 'ai', content, isError, timestamp: new Date().toISOString() }]);
  };

  const addErrorMessage = (message) => addAIMessage(`❌ ${message}`, true);

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // MAIN ANALYSIS HANDLER (UNTOUCHED)
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  const handleSend = async () => {
    ...
    if (inputText.trim()) userMessageParts.push("\n" + inputText.trim());
    ...
    try {
      ...
      if (currentFile) {
        setAnalysisProgress("Uploading document...");
        response = await analyzeDocument(currentFile, (progress) => {
          if (progress.stage === "uploading") setAnalysisProgress("Uploading and extracting text...");
          else if (progress.stage === "analyzing") setAnalysisProgress("Analyzing with AI...");
          else if (progress.stage === "completed") setAnalysisProgress("Finalizing results...");
        });
      } else if (currentText.trim()) {
        setAnalysisProgress("Analyzing text...");
        response = await analyzeText(currentText);
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a0b] text-slate-300 overflow-hidden font-sans">
        ...
        <div className="max-w-3xl mx-auto w-full">
          {chatHistory.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center mt-24 space-y-6">
              ...
              <div>
                <h1 className="text-3xl font-serif font-bold text-white mb-2">Legal AI Analyser</h1>
                <p className="text-slate-500 max-w-sm mx-auto text-sm leading-relaxed">
                  Analyze contracts, identify risks, and simplify legalese with forensic AI precision.
                </p>
              </div>
            </div>
          ) : (
            <div className="space-y-6 pb-10">
              {chatHistory.map((msg, idx) => (
                <div key={idx} className={`flex gap-4 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  {msg.role === 'ai' && (
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 mt-1 ${msg.isError ? 'bg-red-500/20' : 'bg-indigo-600 shadow-lg shadow-indigo-600/20'}`}>
                      {msg.isError ? <AlertCircle size={18} className="text-red-400" /> : <Shield size={18} className="text-white" />}
                    </div>
                  )}
                  <div className={`max-w-[85%] p-4 rounded-2xl text-[14px] leading-relaxed ${
                    msg.role === 'user' 
                    ? 'bg-indigo-600 text-white rounded-tr-none' 
                    : msg.isError ? 'bg-red-500/10 border border-red-500/20 text-red-200' : 'bg-[#121215] border border-white/5 text-slate-300 rounded-tl-none'
                  }`}>
                    <div className="whitespace-pre-wrap break-words">{msg.content}</div>
                  </div>
                </div>
              ...
              {loading && (
                <div className="flex gap-4">
                  <div className="w-9 h-9 rounded-xl bg-[#121215] border border-white/5 flex items-center justify-center shrink-0">
                    <Loader2 size={18} className="animate-spin text-indigo-500" />
                  </div>
                  <div className="px-4 py-3 rounded-2xl bg-[#121215]/50 text-slate-500 italic text-[13px] border border-white/5">
                    {analysisProgress || "Running Forensic Analysis..."}
                  </div>
                </div>
              )}
              <div ref={messagesEndRef} />
      ...
      {/* Input Section */}
      <div className="p-4 sm:p-6 bg-[#0a0a0b]">
        <div className="max-w-3xl mx-auto">
          <div className="bg-[#121215] rounded-2xl border border-white/10 p-2 shadow-2xl focus-within:border-indigo-500/40 transition-all">
            
            {/* Selected File Badge */}
            {file && (
              <div className="mx-2 mb-2 p-2 bg-indigo-500/10 border border-indigo-500/20 rounded-xl flex items-center justify-between text-indigo-400">
                <div className="flex items-center gap-2">
                  <div className="p-1 bg-indigo-500 rounded text-white"><FileText size={12} /></div>
                  <span className="text-xs font-bold truncate max-w-[200px]">{file.name}</span>
                </div>
                <button onClick={removeFile} className="hover:text-red-400 transition-colors"><X size={16}/></button>
              </div>
            )}
            ...
            <div className="flex items-center gap-2">
              ...
              <textarea
                value={inputText}
                onChange={(e) => setInputText(e.target.value)}
                placeholder="Paste legal clauses or ask a question..."
                className="flex-1 bg-transparent border-none focus:ring-0 text-slate-200 py-3 resize-none text-[15px] placeholder:text-slate-600 max-h-32"
                rows={1}
              />
          ...
          <p className="text-[9px] uppercase tracking-[0.2em] text-slate-700 text-center mt-4 font-bold">
            AES-256 Protected Analysis • Nyay Sahayak v1.0
          </p>
        </div>
      </div>
    </div>
