...
const HowItWorks = () => {
  ...
  const steps = [
    { 
      ...
      description: "Securely upload your PDF or image files. Our system immediately masks sensitive data to protect your privacy.", 
      icon: <Upload size={32} /> 
    },
    { 
      title: "Forensic Analysis", 
      description: "Gemini AI scans every clause, identifying hidden risks and translating complex legal jargon into plain English.", 
      icon: <Cpu size={32} /> 
    },
    { 
  ...
  return (
    <section ref={containerRef} className="py-32 bg-transparent relative z-10">
        ...
        <div className="space-y-32">
          {steps.map((step, index) => {
            ...
            return (
              <div 
                key={index}
                className={`step-row flex flex-col ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'} items-center gap-12 md:gap-24`}
              >
                {/* Visual Side with GSAP Parallax */}
                <div className="w-full md:w-1/2 relative">
                  <div className="aspect-square rounded-full bg-gradient-to-tr from-indigo-900/20 to-transparent border border-white/5 flex items-center justify-center relative overflow-hidden group">
                    ...
                    {/* GSAP speed control via data attributes */}
                ...
                <div className="w-full md:w-1/2 text-center md:text-left">
                  <div className="text-indigo-500 font-serif text-2xl font-bold mb-4">
                    Step 0{index + 1}
                  </div>
                  <h4 className="text-3xl md:text-4xl font-bold text-white mb-6">
                    {step.title}
                  </h4>
                  <p className="text-slate-400 leading-relaxed text-lg max-w-md mx-auto md:mx-0">
                    {step.description}
                  </p>
                </div>
              </div>
