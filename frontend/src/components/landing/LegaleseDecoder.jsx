...
const TranslationCard = ({ clause, isActive, direction }) => {
  ...
  return (
    <div
      className={`absolute w-full max-w-md rounded-3xl border p-6 shadow-2xl backdrop-blur-xl transition-all duration-700 ease-out md:p-8 ${clause.cardBg} ${clause.cardBorder} ${
        isActive
          ? 'z-10 scale-100 translate-x-0 translate-y-0 opacity-100'
          : direction === 'up'
          ? 'z-0 -translate-y-12 scale-95 opacity-0'
          : 'z-0 translate-x-12 scale-95 opacity-0'
      }`}
    >
      <div className="mb-6 flex items-center gap-3">
        <div className={`rounded-xl p-3 ${iconBgMap[clause.riskColor]}`}>
          <Icon size={24} />
        </div>
        <div>
